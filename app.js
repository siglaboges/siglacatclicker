...
const TREE = {
  double: {
    name: "Doubles All Income",
    desc: "+1x income",
    x: 700, y: 760, parent: null,
    base: 1, growth: 2, maxLevel: 3, mult: 1,
  },
  sharp: {
    name: "Sharp Claws",
    desc: "+1 click per click",
    x: 460, y: 640, parent: "double",
    base: 1, growth: 2, maxLevel: 3, perClick: 1,
  },
  razor: {
    name: "Razor Claws",
    desc: "+2 click per click",
    x: 260, y: 520, parent: "sharp",
    base: 2, growth: 2, maxLevel: 3, perClick: 2,
  },
  diamond: {
    name: "Diamond Claws",
    desc: "+4 click per click",
    x: 140, y: 380, parent: "razor",
    base: 4, growth: 2, maxLevel: 3, perClick: 4,
  },
  extra: {
    name: "Extra Income",
    desc: "+1x income",
  ...
  nest: {
    name: "Cozy Nest",
    desc: "+1 click per second",
    x: 940, y: 640, parent: "double",
    base: 1, growth: 2, maxLevel: 3, perSecond: 1,
  },
  luxury: {
    name: "Luxury Nest",
    desc: "+2 click per second",
    x: 1140, y: 520, parent: "nest",
    base: 2, growth: 2, maxLevel: 3, perSecond: 2,
  },
  palace: {
    name: "Palace Nest",
    desc: "+4 click per second",
    x: 1260, y: 380, parent: "luxury",
    base: 4, growth: 2, maxLevel: 3, perSecond: 4,
  },
  bonus: {
    name: "Bonus Income",
    desc: "+1x income",
...
function addClick() {
  clicks += (perClick + treePerClick) * mult() * treeMult;
...
function spawnPop() {
  ...
  el.className = "pop";
  el.textContent = "+" + ((perClick + treePerClick) * mult() * treeMult);
...
setInterval(() => {
  if (perSecond + treePerSecond > 0) {
    clicks += (perSecond + treePerSecond) * mult() * treeMult;
    counter.textContent = clicks.toLocaleString();
    updateAll();
  }
}, 1000);
...
function updateCps() {
  ...
  const passive =
    perSecond + treePerSecond > 0
      ? "  +" + ((perSecond + treePerSecond) * mult() * treeMult) + "/s"
      : "";
